...
Your turn:`;
    ...
    try {
      ...
      console.log(`[OpenAI] generateTurn (${model}): ${duration}ms`);

      return response.choices[0]?.message?.content || "";
    } catch (error: any) {
      console.error(`[OpenAI] generateTurn error:`, error);
...
Output ONLY valid JSON, no markdown, no code blocks.`;
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      console.log(`[OpenAI] summarizeJudge (${model}): ${duration}ms`);

      const content = response.choices[0]?.message?.content || "{}";
      const parsed = JSON.parse(content);
      return JudgeSummarySchema.parse(parsed);
    } catch (error: any) {
      console.error(`[OpenAI] summarizeJudge error:`, error);
      throw new Error(`OpenAI API error: ${error.message || "Unknown error"}`);
    }
  }
...
Output ONLY valid JSON, no markdown, no code blocks.`;
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      console.log(`[OpenAI] stanceUpdate (${model}): ${duration}ms`);

      const content = response.choices[0]?.message?.content || "{}";
      const parsed = JSON.parse(content);
      return StanceUpdateSchema.parse(parsed);
    } catch (error: any) {
      console.error(`[OpenAI] stanceUpdate error:`, error);
      throw new Error(`OpenAI API error: ${error.message || "Unknown error"}`);
    }
  }
}
